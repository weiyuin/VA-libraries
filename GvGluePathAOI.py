...
def check_errorcode(str_info,b_show_lackglue=True):
    ...
    try:
        ...
        list_error_code.append(0)#Không có ý nghĩa
    ...
    def show_detetion_Region(self,guiArray,str_info,list_region=None,show_OK=True,font_size=40,line_width=3,b_show_cof=True):
        ...
        for i in range(region_num):
            ...
            try:
                ...
                b_region_enable=False#Không có ý nghĩa
...
=======
﻿# ------------------------------------------------------------------
# File Name:        GvGluePathAOI
# Author:           Hao Zhao, Jun Ling, Huan Wang
# Version:          ver0_4
# Created:          2023/11/21111111
# Description:      用于Airpods标准化检测、数据存储及显示
#                   glue_display:        标准显示类
#                   detection_operator： 缺陷再运算类(客户定制需求时使用)
#                   database_operator：  数据存储类
# History:
#       <author>        <version>       <time>      <desc>
#       Huan Wang        ver0_1         2023/10/15  初始版本
#       Jun Ling         ver0_2         2023/11/15  修改数据存储使能逻辑,增加数据统计等功能
#       Hao Zhao         ver0_3         2023/11/15  增加标准化显示,检测数据刷新等功能,增加示例,标准模板定版
#       Hao Zhao         ver0_4         2023/11/21  兼容拓展区域只检胶占比忽略其它
#       Hao Zhao         ver0_5         2023/11/22  修改glue_missing比较方式
#       Huan Wang        ver0_6         2023/12/05  增加mes上传功能
#       Hao Zhao         ver0_7         2023/12/09  基于37需求修改显示内容；修改Add函数（溢胶，多/少胶,断胶，胶长）；修改merge不同个数区域可进行合并
#       Huan Wang        ver0_8         2023/12/17  增加mes上传shiftX/shiftY功能
#       Hao Zhao         ver0_10        2024/1/03   基于37需求修改显示内容：cof不显示在NG原因中，增加配置取消最小/最大胶长的显示，COF显示OK
#       Huan Wang        ver0_11        2024/1/03   mes上传width数据修改为哪个NG上传哪个
#       Hao Zhao         ver0_12        2024/1/22   合并36/37上传功能
#       Hao Zhao         ver1_13        2024/3/22   增加37类项目显示用，用于显示常规信息：物料结果,SN,NG原因，最短胶宽，详细结果信息（提示区域）
#       Huan Wang        ver1_14        2024/4/8    修改上传保留小数点个数
#       Hao Zhao         ver1_15        2024/4/27   去掉上传数据COF检测项中"cof_"前缀；37项目将glue missing 修改为glue area；修复溢胶显示中溢胶区域统计异常导致溢胶无法显示
#       Hao Zhao         ver1_16        2024/7/31   1.上传部分去掉Gross SPEC为工具0.5的方式(建议后期软件可访问)
#                                                   2.显示部分去掉不常用控制变量，同时合并部分功能重合的控制变量
#                                                   3.新增显示函数兼容一般信息，检测区域，NG信息显示，简化显示调用过程
#                                                   4.新增检测结果返回函数，不用了解胶检数据结构直接获取结果便于存图路径调用等
#                                                   5.新增上传数据处理函数，以要求上传格式兼容当前机台工程内部处理，简化工程
#       Hao Zhao         ver1_17        2024/11/08  1.修复溢胶轮廓显示error_code使用错误情况
#                                                   2.上传格式部分，去除回流补交对结果判断逻辑
#                                                   3.更新示例用法
#       Hao Zhao         ver1_18        2024/11/18  1.新增NoGlue检测项显示
#                                                   2.新增NoGlue检测项结果返回
#                                                   3.统一工具检测NG时显示NG，统一GlueMissing显示
#                                                   4.增加断胶显示屏蔽功能
#       Hao Zhao         ver1_19        2024/11/30  1.新增函数get_all_detection_result返回所有检测项结果，便于后期自定义显示内容
#                                                   2.新增显示show_detection_result，匹配39客户指定新的显示模板
#                                                   3.新增ShowFeatureVec函数，便于多穴特征显示
#       Hao Zhao         ver1_20        2024/12/05  1.新增函数get_mes_data_Ex，简化39上传模式
# ------------------------------------------------------------------
# example
# 主逻辑
"""
...
str_2 = GvTool.GetToolData("主胶路覆盖率检测_4692.缺陷序列化结果")
#数据序列化--------------------------------------------------------
str_1=json.loads(str_1)
str_2=json.loads(str_2)
#数据融合
str_out = result_opr.merge(str_1,str_2)
#数据修改
str_out = result_opr.data_replace(str_out,[0.2],[0],4)#孔洞/占比项，修改数据后OK是0，NG直接为1；其他项，修改数据后OK是0，NG为2（若只修改孔洞，占比后无需再调用缺陷再计算，其他一定需要）

# 2. 缺陷追加(如果有)
# str_3 = GvTool.GetToolData("溢胶检测_4693.缺陷序列化结果")
# str_out = result_opr.add(str_out,str_3)
# str_outtemp=json.loads(str_1)
# print(str_outtemp["base_info"]["error_code"])

# 3. 缺陷再计算
thred_miss = GvVar.GetVar("@dMissingThreshold")
thred_break = GvVar.GetVar("@dGlueBreakThreshold")
thred_hole = GvVar.GetVar("@dHoleThreshold")
thred_coverage = GvVar.GetVar("@dGlueCoverageThreshold")
#显示及修改的区域
list_region=[0,1,2,3,4,5]
str_out=result_opr.recalculate_result(str_out,thred_miss,thred_break,thred_hole,thred_coverage,list_region)
# ----------------------------------------------------------------
...
guiArray = gui_opr.show_NG_data(guiArray,str_out,5,20,100,-40,0)
#################方式2
guiArray = gui_opr.show_general_data_Ex(guiArray,str_out,font_size=70,offset_x=20,offset_y=20,line_space=10,line_width=2,bShowMinWidth=True)#直接显示全部，信息头，NG信息，检测区域
# ----------------------------------------------------------------
# 存储
db_opr = GvGluePathAOI.database_operator()
str_file_path = GvVar.GetVar("@filePath")
str_path_cap = GvVar.GetVar("#strImgPathCapSnd")
str_path_src = GvVar.GetVar("#strImgPathSrcSnd")
...
class glue_display():
    ...
    #用于显示常规信息：物料结果,SN,NG原因，最短胶宽，详细结果信息,检测区域等
    #guiArray :显示用GuiArray
    #str_info :异型胶输出数据
    #font_size：显示字体
    #offset_x：显示偏移量X
