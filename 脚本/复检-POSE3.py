...
str_out1=json.loads(GvTool.GetToolData("异形胶检测工具_5344.缺陷序列化结果"))
str_out2=json.loads(GvTool.GetToolData("异形胶检测工具_5345.缺陷序列化结果"))
# 百分之百1
Cover_rate1 = 1
if GvTool.GetToolData("Blob工具_5778.执行结果") and GvTool.GetToolData("百分之百1_5789.总像素数") > 0 :
    gap_all = GvTool.GetToolData("百分之百1_5789.总像素数")
    gap_del = GvTool.GetToolData("Blob工具_5778.总像素数")
    Cover_rate1 = 1 - gap_del/gap_all
    if gap_del > 0:
        # 画图
        if Cover_rate1 > 0.95:
            clr = [0,255,0] #绿色
            ...
            clr = [255,0,0] #红色
        for i in GvTool.GetToolData("Blob工具_5778.Blob结果轮廓"):
            guiArray.Add(ShowLineSeg(i,clr))
else:
    Cover_rate1 = 1
# 百分之百2
Cover_rate2 = 1
if GvTool.GetToolData("Blob工具_5794.执行结果") and GvTool.GetToolData("百分之百2_5790.总像素数") > 0 :
    gap_all = GvTool.GetToolData("百分之百2_5790.总像素数")
    gap_del = GvTool.GetToolData("Blob工具_5794.总像素数")
    Cover_rate2 = 1 - gap_del/gap_all
    if gap_del > 0:
        # 画图
        if Cover_rate2 > 0.95:
            clr = [0,255,0] #绿色
            ...
            clr = [255,0,0] #红色
        for i in GvTool.GetToolData("Blob工具_5794.Blob结果轮廓"):
            guiArray.Add(ShowLineSeg(i,clr))
else:
    Cover_rate2 = 1
# 百分之60
Cover_rate3 = 1
if GvTool.GetToolData("Blob工具_5796.执行结果") and GvTool.GetToolData("百分之60_5791.总像素数") > 0 :
    gap_all = GvTool.GetToolData("百分之60_5791.总像素数")
    gap_del = GvTool.GetToolData("Blob工具_5796.总像素数")
    Cover_rate3 = 1 - gap_del/gap_all
    if gap_del > 0:
        # 画图
        if Cover_rate3 > 0.6:
            clr = [0,255,0] #绿色
            ...
            clr = [255,0,0] #红色
        for i in GvTool.GetToolData("Blob工具_5796.Blob结果轮廓"):
            guiArray.Add(ShowLineSeg(i,clr))
else:
    Cover_rate3 = 1
str_out2 = result_opr.data_replace(str_out2,[Cover_rate1,Cover_rate2,Cover_rate3],[0,1,2],5)
print([Cover_rate1,Cover_rate2,Cover_rate3])

str_out3=json.loads(GvTool.GetToolData("异形胶检测工具_5785.缺陷序列化结果"))
str_out=result_opr.add(str_out1,str_out2)
str_out=result_opr.add(str_out,str_out3)
...
## NoGlue结果赋值
if GvTool.GetToolData("Blob工具_5343.总像素数")>6000:
    noGlueState=1
else:
    noGlueState=0
...
GvGuiDataAgent.SetGraphicDisplay("工位3-3复检", guiArray)  
## 检测结果赋值
error_code,strRes,strResMes = gui_opr.get_detect_result(str_out)
GvVar.SetVar("#strFlag",strRes)
print(strRes)
## 检测数据回传
GvTool.SetToolData("异形胶检测工具_5344.缺陷序列化结果",json.dumps(str_out))
