...
str_out1=json.loads(GvTool.GetToolData("异形胶检测工具_5456.缺陷序列化结果"))
str_out2=json.loads(GvTool.GetToolData("异形胶检测工具_5457.缺陷序列化结果"))
...
## NoGlue结果赋值
if GvTool.GetToolData("Blob工具_5455.总像素数")>6000:
    noGlueState=1
else:
    noGlueState=0
...
GvGuiDataAgent.SetGraphicDisplay("工位3-5复检", guiArray)  
## 检测结果赋值
error_code,strRes,strResMes = gui_opr.get_detect_result(str_out)
GvVar.SetVar("#strFlag",strRes)
print(strRes)
## 检测数据回传
GvTool.SetToolData("异形胶检测工具_5456.缺陷序列化结果",json.dumps(str_out))
