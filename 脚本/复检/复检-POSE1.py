...
str_out11=json.loads(GvTool.GetToolData("异形胶检测工具_5500.缺陷序列化结果"))
str_out12=json.loads(GvTool.GetToolData("异形胶检测工具_5501.缺陷序列化结果"))
str_out1=result_opr.merge(str_out11,str_out12)
str_out21=json.loads(GvTool.GetToolData("异形胶检测工具_5753.缺陷序列化结果"))
str_out22=json.loads(GvTool.GetToolData("异形胶检测工具_5754.缺陷序列化结果"))
str_out2=result_opr.merge(str_out21,str_out22)
...
## NoGlue结果赋值
if GvTool.GetToolData("Blob工具_5499.总像素数")>6000:
    noGlueState=1
else:
    noGlueState=0
...
guiArray = gui_opr.show_detetion_Region(guiArray,str_out12)#显示胶占比ROI
guiArray = gui_opr.show_detetion_Region(guiArray,str_out22)#显示胶占比ROI
GvGuiDataAgent.SetGraphicDisplay("工位3-1复检", guiArray)  
## 检测结果赋值
error_code,strRes,strResMes = gui_opr.get_detect_result(str_out)
GvVar.SetVar("#strFlag",strRes)
print(strRes)
## 检测数据回传
GvTool.SetToolData("异形胶检测工具_5500.缺陷序列化结果",json.dumps(str_out))
